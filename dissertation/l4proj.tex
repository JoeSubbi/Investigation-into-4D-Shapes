...
\vskip 0.5em
%Texturing
Each extension to the 3D cross section took advantage of colouring different shapes within a scene. To colour different shapes within a scene a second function is employed that for the most part is identical to the SDF describing the scene, however it returns a number. Each value the number may take is a material index. To find what material number to set each object in the scene, a comparison is made between the distance returned by the union of all SDFs in the seen, and the distance of a specific SDF in the scene. If they are equal, then you may return the material index you intend to associate with that specific SDF \citep{the_art_of_code_setting_2021}.
\vskip 0.5em
...
% - what it will not provide benefit with
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{The projection of multivectors in $\mathbb{R}^3$ onto their basis blades. \subref{fig:vec_proj} shows the projections of a vector as projections stemming from the }
