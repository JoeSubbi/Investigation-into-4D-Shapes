...
TODO: Describe Sphere SDF

\begin{lstlisting}[language=c, caption={The signed distance function for a sphere in $\mathbb{R}^3$}, label=lst:sdSphere]
...
TODO: Summarise SDFs in more detail - shapes defined with signed distance functions

relative to the camera - slice is based on W coordinate of camera based

TODO: Discuss extending raymarch algorithm to 4D (extend SDFs to 4D)

TODO: Discuss lighting the objects

The shader can be configured such that if the ray does not make contact with a surface the pixel is set to a default colour or it can be discarded. 

\vskip 0.5em

\subsection{Derivation of 4D Shapes}

subtracting the distance adds thickness
...
The signed distance function referenced in the ray marching algorithm defines the scene to be rendered. In this function, several other SDFs can be referenced to render several objects at once
