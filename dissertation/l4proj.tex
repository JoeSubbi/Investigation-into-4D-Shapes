...
%correct over time
As the participant spends more time dealing with 4D shapes, they become more familiar with these shapes. As shown in  \cref{fig:shape_stats}, participants tend to answer more accurately over time; where each iteration follows the introduction of a new representation in random order.
...
\vskip 0.5em
% Representations
The 4D to 3D rotational abstraction performed better than any other representation for all three metrics: totally correct answers, similarity of an answer, and the time taken to answer (\cref{fig:rot_stats}). Whilst this representation proved most effective at conveying the active planes of rotation, there is a debate to be had whether a user understood what they were seeing on screen, or if the representation just gives the answer away without the need for critical thinking.
\vskip 0.5em
On the other hand, the multi-view representation has a notably lower quantity of totally correct answers compared to the other representations. This trend can also be seen when looking at the jaccard index. Discussion with participants yielded a common theme: Given four objects rotating on screen, where some rotate in 3D and some in 4D, participants were often mislead, seeing patterns in objects rotations, but drawing the incorrect conclusions. It would seem that it takes a more professional eye, and more time and experience with the representation to take advantage of it. Participants also took a longer time to answer when using the multi-view representation. 
\vskip 0.5em
The w-axis timeline did not seem particularly better or worse than the control representation. The timeline representation does not provide any extra information on the rotations of a 4D object, and as such has not seemed to benefit users in interpreting the active planes of continuous rotation.
