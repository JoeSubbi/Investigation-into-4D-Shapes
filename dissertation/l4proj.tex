...
%
Each cross section, as they move further away from the central slice, decreases in size; to avoid potential confusion in conveying the idea that travelling across the fourth dimension wraps around, forming a circular path.
%
...
Secondly, complex objects such as the 120-cell, contain thousands of edges and hundreds of vertices. Viewing the surface of this object likely allows users a better understanding of the geometry.
%
With the goal of teaching others, \citet{matsumoto_polyvision_2019} restricts interaction to that which we are capable of. Translation, rotation and scaling are restricted to the third dimension. Interaction in $\mathbb{R}^3$, from another perspective in $\mathbb{R}^4$ may change the object in a potentially unexpected way. Whilst the effects of 4D manipulation can be seen in this way, \textit{Polyvision} lacks the ability to directly manipulate an object in 4D space.
...
Unfortunately, given the limited time to conduct the investigation, and that of an individual experiment, participants will not be able to sufficiently progress through each level, whilst also covering each of the five stages of the learning process. 
...
%correct over time
As the participant spends more time dealing with 4D shapes, it makes sense that they become more familiar with these shapes. As shown in  \cref{fig:shape_stats}, participants tend to answer more accurately over time; where each iteration follows the introduction of a new representation in random order.
%submission time over time
Following this, \cref{fig:shape_stats} also shows that participants tend to submit their answer earlier with each iteration of a new representation.
\vskip 0.5em
...
These two elements are backed up by the time taken per representation (\cref{fig:shape_stats}), where participants were much more likely to expire the twenty second time limit when interacting with representations rendering more content.
\vskip 0.5em
...
Encouragingly, a positive correlation can be seen between a participants correct answers and their confidence and understanding of their submission (\cref{fig:shape_corr}). Their confidence was rated on a scale of 0 to 10, and their understanding was a binary answer of "yes" or "no".
This correlation suggests that participants were not making blind guesses.
\vskip 0.5em
% time taken
A strong correlation can be seen between a participants confidence and how long they took to answer the question. As expected, a participant took longer to submit their answer if they were unsure of the correct answer.
...
%
% per iteration
With each iteration of a new representation, there appears to be a steady increase in accuracy, indicated by the quantity of perfect answers, and the similarity using the jaccard index (\cref{fig:rot_stats}).
Furthermore, the time taken to answer clearly decreases as users spend more time with the 4D shapes.
\vskip 0.5em
%

TODO: rotation matching
