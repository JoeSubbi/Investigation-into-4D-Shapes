...
TODO: reiterate data collected

time, accuracy, initial rotation - distance from 90 degrees, change with each iteration of representation. representation, shape, enabled roations.
...
TODO: Cone and capsule coded with 50\% of being un-rotated, encourage users to explore with w axis and rotation
...
performance of WebGL application
...
timeline is worst
 - should have been better than control or multi-view
 - recurring theme - limited data may not be very representative
 - note - more shapes on screen, more lag - this rep had most shapes on screen of any other. 

more correct over time

faster response over time, first 2 attempts were jarring with short limit

less info more correct

longer to answer with more info

control took this least time - further indicating less info can be better
...
confidence and understanding correlated with correctness - not just blind guesses

correctness would decrease with increase in time - unanswered questions, prob with experiment

less correct with cone - if unrotated, may be considered a sphere?

torus more correct - quite an obvious shape

users would interact a lot more with sphere to verify it is a sphere

confidence at highest with control rep - less info

bigger focus on rotating object with multiview

less rotations with 4D-3D for some reason? weird, would have thought it would be more similar to control
...
\citep{glen_jaccard_2022} jaccard index
...
increase over time, but interaction 2 anomoly
