...
The material received by the students at this stage was compiled in the form of a video tutorial.
\vskip 0.5em
...
The 2D cross sections of some primitive 3D shape, namely the hyper-sphere and a hyper-cube, were showcased; with parallels drawn between the 3D cross sections of analogous 4D shapes. 
To complement the idea of the 3D cross section, the geometry of a hyper-sphere and a hyper-cube were described independent to their cross sections.

The rotation of 4D geometry is a more complex subject. As such, the rotation was broken down into several stages in and of itself.
Rotation in \(n\) dimensions must be considered within dimensions, as stated earlier. That is, a rotation must be considered a planar rotation, opposed to axial rotations as most people are used to. 
Given axial rotations is the common understanding, planar rotations are first introduced in 3D, equated to the axial rotations.
To emphasise how planar rotations work in \(n\) dimensions, a 2D cross section of a 3D object is first rotated within the two dimensions of the slice of 3D space. This showcases that an object rotating within a cross section does not change size or shape. On the other hand, a 2D slice of a 3D object, rotating in three dimensions  (a rotation involving the $z$ axis) will change size and shape. Following this, the parallels are then drawn between the 3D cross section of a 4D object: The 3D cross section will not change size or shape for any rotation within three dimensions, however it will if rotating within four dimensions (a rotation involving the $w$ axis).
...
To elaborate on the participants understanding, following the tutorial video, an informal discussion would take place. This discussion would utilise the \textit{explanation or explication} and \textit{integration} phases of the learning process; where a student is asked to formulate what they have learned, and share their opinions on the relationships they have discovered. Here the teacher may correct terminology. Towards the end of the discussion, without any addition of new material, the participant would summarise what they have learned to ensure ensure adequate preparation for the coming experiment.
\vskip 0.5em
Given the limited time to conduct an experiment, the remainder of the experiment took guidance from both the \textit{free orientation} and \textit{guided or directed orientation} stages of van Hiele's learning process, effectively merging them into a single orientation phase.  
%play with a 4D shape, get the hang of controls - free orientation
Prior to the main portion of the experiment, participants were provided with a 4D hypercube, and a summary of the controls to manipulate the shape. Here a participant may spend time observing how the shape may change when undergoing different rotations, as well as how the shape may change when moving it through the hyper plane. 
Informal tasks were proposed to the participants in an attempt to allow them to connect relationships of the geometry. An example of such a task included: Rotating the shape with the use of a 4D rotation such that the colours of two opposing faces of the 3D cross section switch sides of the object. Here participants may learn basic properties of four dimensional rotations, and should be able to connect the similarities to rotating a shape in $\mathbb{R}^3$.
\vskip 0.5em
...
First, the participant was be tested on their understanding of the surface of a shape. Given a cross section of a 4D object, the participant should be able to identify the shape on screen. The participant was allowed to manipulate the object to assist them given a limited time frame.
